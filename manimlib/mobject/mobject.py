...
class Mobject(object):
    ...
    """
    这里的几个参数给人很多启发。
    - ``reflectiveness`` : 反射度，越大越亮
    - ``shadow`` : 阴影，越大越暗
    - ``gloss`` : 光泽，越大越亮
    上面几个参数是和光源有关的，可以用来模拟光照效果。

    只要设置了光源，就可以产生光照效果。
    以前一直不明白光照的效果需要在视频代码中一行行写吗？
    现在明白了，只要设置了光源位置，库本身完成了光照的计算。
    对于使用库的人来说，只需要设置光源位置，就可以产生光照效果。

    库的作用就是把一些常用的功能封装起来，让使用者更方便地使用。
    3b1b能够实现光照效果，真是genius。
    """
    ...
    def __init__(self, **kwargs):
        ...
        self.family: list[Mobject] = [self] # 什么是 family？
        self.locked_data_keys: set[str] = set()
        self.needs_new_bounding_box: bool = True # 每一个物件都有一个包围框
    ...
    # self.color和self.opacity两个属性在类中好像没有。
    # 注意到这两个属性在CONFIG中，所以这里是从CONFIG中获取的。
    def init_colors(self):
        self.set_color(self.color, self.opacity)

    # 所有的mobject都是由点组成的
    # 比如说一条曲线，可以等间隔的生成曲线上的点，然后用这些点来绘制曲线
    # 注意，这里的点就是实打实的曲线上的点，而不是贝塞尔曲线上的点
    # 在库里搜索init_points，可以看到有很多子类重写了这个方法
    ...
    """
    The copy() function in Python is a method that is used to create shallow copies of objects, 
    such as lists, dictionaries, or sets. A shallow copy means that the new object 
    references the same elements as the original object, but does not create new copies of them. 
    This means that if you modify an element in the original object, it will also affect the 
    copied object, and vice versa.
    """
    def set_uniforms(self, uniforms: dict):
        '''设置 uniform 变量，以字典形式传入'''
        for key, value in uniforms.items():
            if isinstance(value, np.ndarray): # 为何要判断是否为np.ndarray？
                value = value.copy() # 浅拷贝
    ...
    def set_points(self, points: npt.ArrayLike):
        '''设置锚点'''
        # 个人觉得，这里的注释有问题
        # 在manimgl中，曲线都是由多段二阶贝塞尔曲线组成的
        # 这里的points的数量是3的倍数，因为二阶贝塞尔曲线需要两个anchor和一个handle
        # 所以，这里的set_points函数的作用是设置anchor和handle
        if len(points) == len(self.data["points"]):
            self.data["points"][:] = points # in place更新
        elif isinstance(points, np.ndarray):
            self.data["points"] = points.copy() # 这里为何要copy？直接赋值不行吗？
        else:
    ...
    def apply_points_function(
        ...
        works_on_bounding_box: bool = False # 是否作用于包围框
    ):
    ...
    # 物体所在的空间是无限大的。你可以把物体放在空间中的任何位置
    # 但是，照相机能够看到的空间是有限的
    # 一个有关frame width和pixel width的猜想
    # frame width是物理上的尺寸，单位可能是英寸
    # pixel width是像素的数量，单位是像素
    # 当我们设置一个坐标轴对象的时候，需要指定width和height
    # 这个width和height的单位就是frame width的单位
    # 可以写一个代码测试一下
