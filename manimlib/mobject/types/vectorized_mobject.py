...
class VMobject(Mobject):
    ...
    def set_anchors_and_handles(
        ...
        nppc = self.n_points_per_curve # CONFIG字典中给出的是3，二阶贝塞尔曲线有两个anchor和一个handle
        # 这里有一个猜想
        # 第一条二阶贝塞尔曲线的最后一个anchor是第二条二阶贝塞尔曲线的第一个anchor
        # 那么，就有两种存储方法
        # 1.重合的anchor只存储一次
        # 2.重合的anchor存储两次
        # 在这里，选择了第二种方法
        # 但是，在最新版的manimgl（v1.6.1），可以发现，选择了第一种方法（优化了）
        new_points = np.zeros((nppc * len(anchors1), self.dim)) # 这里就能理解了，为什么要乘以nppc
        arrays = [anchors1, handles, anchors2]
        for index, array in enumerate(arrays):
            new_points[index::nppc] = array # new_points中重复存储了部分anchor
    ...
    def pointwise_become_partial(self, vmobject: VMobject, a: float, b: float):
        ...
        if a <= 0 and b >= 1: # 从这里看，a和b最好是介于0和1之间的小数
            self.become(vmobject)
            return self
        num_curves = vmobject.get_num_curves() # 构成整条曲线的贝塞尔曲线的数量
        nppc = self.n_points_per_curve # 每段贝塞尔曲线的点数
